...
import type { HoriLoadingOptions } from "./HoriLoadingOptions";
...
export class SolidParser {
  ...
  public static NormalPattern =
    /^vn(\s+[\d|.|+|\-|e|E]+)( +[\d|.|+|\-|e|E]+)( +[\d|.|+|\-|e|E]+)/;
  ...
  public constructor(
    ...
    loadingOptions: HoriLoadingOptions
  ) {
   ...
   */
  private _isInArray(
    arr: Array<{ normals: Array<number>; idx: Array<number> }>,
    obj: Array<number>
  ) {
    if (!arr[obj[0]]) {
  ...
  private _setData(
    ...
    if (_index === -1) {
      ...
      this._tuplePosNorm[indicePositionFromObj].normals.push(
        indiceNormalFromObj
      );
      this._tuplePosNorm[indicePositionFromObj].idx.push(
        this._curPositionInIndices++
      );
      if (this._loadingOptions.optimizeWithUV) {
  ...
  private _unwrapData() {
    ...
    for (let l = 0; l < this._wrappedPositionForBabylon.length; l++) {
      ...
      this._unwrappedUVForBabylon.push(
        this._wrappedUvsForBabylon[l].x,
        this._wrappedUvsForBabylon[l].y
      ); //z is an optional value not supported by BABYLON
    ...
    for (let k = 0; k < this._triangles.length; k++) {
      ...
      this._setData(
        ...
        this._loadingOptions.importVertexColors
          ? this._colors[indicePositionFromObj]
          : undefined
      );
    }
  ...
  public parse(
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.length === 0 || line.charAt(0) === "#") {
          ...
          if (result.length >= 7) {
            ...
            this._colors.push(
              new Color4(
                ...
                g > 1 ? g / 255 : g,
                b > 1 ? b / 255 : b,
                result.length === 7 || result[7] === undefined
                  ? 1
                  : parseFloat(result[7])
              )
            );
        ...
        SolidParser.GroupDescriptor.test(line) ||
        SolidParser.ObjectDescriptor.test(line)
      ) {
        ...
        if (!this._isFirstMaterial || !this._hasMeshes) {
          ...
          const objMesh: MeshObject =
            ...
            {
              name:
                (this._objMeshName || "mesh") +
                "_mm" +
                this._increment.toString(), //Set the name of the current obj mesh
              indices: undefined,
              positions: undefined,
              normals: undefined,
              uvs: undefined,
        ...
        if (this._hasMeshes && this._isFirstMaterial) {
          ...
          this._meshesFromObj[this._meshesFromObj.length - 1].materialName =
    ...
    for (let j = 0; j < this._meshesFromObj.length; j++) {
      ...
      if (this._loadingOptions.computeNormals) {
        ...
        VertexData.ComputeNormals(
          this._handledMesh.positions,
          this._handledMesh.indices,
          normals
        );
